...
describe('extractor', () => {
  it('should extract the job requirements', async () => {
    const jobPdfFilePath = './libs/utils-lib/src/lib/requirement/extractor/pdf-examples/sample-job-description.pdf';
    const jobRequirements = getRequirementsFromJobDescriptionPdfFile(jobPdfFilePath, Model.GPT_4_TURBO)
    ...
    expect((await jobRequirements).otherQualifications.length).toStrictEqual(0);

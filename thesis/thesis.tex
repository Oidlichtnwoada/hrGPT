...
Furthermore, the model is planned to supply an explanation for each score output that clarifies why it thinks this score is justified.
...
It must be noted that a promising candidate can have a lower total score than a non-promising candidate. Whether a candidate is promising is not solely based on the total score but also on the individual scores of the requirements and whether they are mandatory or not.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        One recruiter mentioned that categorizing and ranking low-skilled jobs may be a significant challenge for the model due to the low-quality applicant documents in this domain.
        ...
        One recruiter suggested that the model should have a dedicated pipeline step to augment the found job requirements with implicit job requirements that are not explicitly given in the job description. This could be done by querying the Internet for the job title and the company name and analyzing the job description for implicit requirements. This recruiter mentioned that this step is crucial as the model may not be able to find all implicit requirements by analyzing the job description alone.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    Furthermore, it was proposed to make the job requirement type weightings more fine-grained by introducing subtypes for each type. This would allow for a more precise configuration of the model, but it needs a predefined or user-defined ontology for each type to define the relations.
    \item \textbf{Implicit Job Requirements}\\

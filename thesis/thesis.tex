...
\begin{enumerate}
    ...
    Prospective applicants needed to go to the company to manually turn in a paper job application.
...
\begin{enumerate}
    ...
    actually apply. Machine learning can help refine the job description in ways that attract more applicants or bring more balance to the gender of the applicants. As there are more passive candidates not actively searching for a job than actively searching candidates, machine learning can be used to identify suitable talents from a massive pool of candidates, e.g., hundreds of millions of \textit{LinkedIn} users.
...
That means companies of any size can use this tool and integrate it into their human capital sourcing pipelines to eventually increase their efficiency and decrease costs.
...
\begin{itemize}
    ...
    Static job analysis is flawed in many industries as jobs are dynamic and ever-evolving. Therefore, it is better to recruit a flexible workforce that is capable of teamwork.
...
\begin{itemize}
    ...
    \end{figure}
    This taxonomy would today be labeled as a knowledge graph, which is a way to structurally store information in graphs.
...
The second approach can be used to further assess the candidates that the proposed model from the thesis finds promising, and the third approach is also necessary to attract a diverse set of applicants in the first place.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        This could maybe be done in such a way that every user is capable of tweaking the configuration of the system.
...
As this conversion is not the focus of this thesis, it will be only briefly discussed in the chapter \ref{implementation}.
...
\begin{itemize} \label{job_requirement_types}
    ...
    This requirement type includes knowledge of specific fields that are necessary to perform the job, which is not listed in the other requirement types.

...
In this section, the literature on how the person-environment fit can be quantized in a technical perspective is discussed.
...
It is described how the input screening documents are processed step by step to the final model output.
Furthermore, it is explained what \acs{LLM}s are supported to be used within the proposed model and there are also some remarks made on the context windows and determinism of \acs{LLM}s.
Moreover, the fully changeable configuration of the proposed model is presented, its scope and limitations as well as the model's capability to handle various input and output languages and different input document formats.
...
In order to lead the discussion for both meetings in the right direction, a discussion guide was used to formulate the main topics and the most important open questions in the form of a questionnaire.
...
The experiment results (applicant ranking, applicant promising categorization and human expert time usage) were typed into the below online form by the each human expert recruiters participating in the experiment.
...
This section covers what data was collected.
The two meetings that were used to get qualitative data were categorized and coded with qualitative content analysis with inductive category formation according to \textit{Mayring} \parencite{mayring}.
The numeric data from the experiment that was submitted with the online form was aggregated according to section \ref{quantitative_research_design}.
...
The following two section analyze the accuracy of the model for the applicant ranking task and the applicant categorization task that divides the applicants into promising and non-promising candidates.

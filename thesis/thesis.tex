...
\end{itemize}
By analyzing all the discussed survey instruments, it was decided that the most suitable quantitative survey instrument for the thesis' case is the research experiment where results are communicated using an online form in order to sanity check the passed input data.
...
The participating five human expert recruiters where affiliated with the following five anonymized recruiting companies:
\begin{enumerate}
...
The accuracy of the designed screening model will be determined by comparing the human expert annotations to the model's output for the same sample screening documents and the following labels:
\begin{enumerate}
    \item \textbf{Ranking}\\
...
The time savings associated with the model usage will be determined by analyzing the time usage of the human expert recruiters while carrying out the applicant ranking and categorization task.
Since the proposed model can run asynchronously in the background whenever new applicant documents come in, the time usage of the model itself during its execution is almost never a limiting factor for the productivity of employees in the \acs{HR} sector.
The model will just add some latency to incoming applicant documents, which are then preprocessed and easier to work with for the human expert recruiters.
This latency is below one minute per applicant documents, but exact runtimes of the model will be sampled and reported.

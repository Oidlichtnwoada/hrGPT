...
This section relies heavily on the article \parencite{ai_recruiting} which is based on over $50$ research papers in that field and which describes the evolution of the recruiting in a very concise way.
As described in \parencite[1]{ai_recruiting}, the average firm's value by 2000 was roughly consisting of 65\% of value from intangible assets.
...
The article \parencite[2-4]{ai_recruiting} described four evolution stages of recruiting which are the following:
\begin{enumerate}
    ...
    But as described in \parencite[2]{ai_recruiting}, in analog recruiting these two goals faced the analog reach and richness frontier as shown in \ref{fig:analog_reach_richness_frontier} because the more information is supplied, the more costly it is when you supply to media with great reach at that time:
    \begin{figure}[H]
        \centering
    ...
    Moreover, research showed that top performers are four to eight times more productive that the average performers, increasingly so in complex environments, according to \parencite[4]{ai_recruiting}.
    Furthermore, this new evolution stage of recruiting also has its downsides as discussed in \parencite[4-5]{challenges_opportunities_hr} due to the emerging use of technology:
    \begin{itemize}
...
As described in \parencite[1]{gpt3}, the model provided very good performance on various \acs{NLP} datasets that include tasks like translation and question answering.
...
The tokenizer that OpenAI is using, can be accessed via this website \parencite{openai_tokenizer}.
...
For this section lots of citations are made from the article \parencite{po_and_pj_fit_literature_review} as it is a very good summary of more than $90$ research papers in this field.
...
Person-organization fit can be operationalized in four different ways according to \parencite[182]{po_and_pj_fit_literature_review}. The following list is ordered from most used to least used operationalization:
\begin{enumerate}
...
\begin{enumerate}
    ...
    The comparison of the rankings of the model against the mean or individual rankings of the human experts will be done using the rank-biased overlap metric which was introduced in \parencite{rank_biased_overlap}. 
...
Rank-biased overlap (RBO) was used in \parencite{rank_biased_overlap} to compare the search results of various search engines.
...
\end{itemize}
As described in \parencite[1]{rank_biased_overlap}, it was the first metric at that time that had the following three properties when comparing incomplete rankings (incomplete means that not every element from the population must be necessarily ranked):
\begin{enumerate}
...
\lstinputlisting[language=TypeScript,label=lst:code_usage_example,caption=Example usage of the programmed model]{../libs/utils-lib/src/lib/wrapper/wrapper.spec.ts}

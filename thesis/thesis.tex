...
This means that the model will not be able to give a plausible output on every theoretically possible resume-job-matchup and should therefore not be used as a single point of truth to prefilter applicants in the screening process if fairness is a top priority.
...
\begin{enumerate}
\item Nonconjointness (incomplete rankings must not necessarily contain the same elements from the population)

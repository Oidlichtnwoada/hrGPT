...
Most of the results have already been interpreted in the previous chapter. This section quickly summarizes the use cases of the proposed model as suggested by the human recruiters. The human recruiters would only categorize applicants with an assistive \acs{AI} tool in order to double-check the results. The transparency of the model was well received by the recruiters and they would not only use it for incoming application, but also as a way to source applicants from already available databases as a sort of an \acs{AI} filter mechanism. As the current state of the model has no support for multimodal input that could be a possible area for improvement in the future. The recruiters gave also very practical recommendation that the model should spit out follow-up points that should be discussed with the applicant in direct communication. The interactive work with the recruiters was increadibly helpful and sparked lots of improvment ideas by reciprocal communication. 

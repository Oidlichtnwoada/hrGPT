...
The \acs{TAPJFNN} framework supports predicting person-job fit predictions, talent sourcing tasks and job recommendation tasks \parencite[1]{pj_fit_ml}.
...
It is proposed in \textcite[2]{pj_fit_ml} that the job description is represented as a set of technical skill requirements and a set of comprehensive quality requirements.
The applicants's abilities are extracted from the applicant's documents and are matched against the two requirement sets by using a weighted, accumulative score, as each requirement can be covered by several abilities and most of the applicants will have different abilities. 
...
Also the constructed keyword features also need manual labor which is a major downside \parencite[5]{pj_fit_ml}.
...
\subsubsection{Person-Job Fit Quantization Problem}
...
\begin{itemize}
    ...
    \item $w^J_{l,t}$: work embedding of the 
\end{itemize}

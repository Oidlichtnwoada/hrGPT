...
This section gives an overview on how the evolution of digital recruiting has led to much more candidates applying to each job offer \parencite[4]{ai_recruiting}.
It also discusses that the use of \acs{AI}-enabled tools in human resources is increasing and that mostly larger companies can afford to use these tools or have the required amount of historic recruiting data to calibrate the tools.
...
To bring a part of this efficiency gain also to smaller companies, this thesis aims to implement an open-source \acs{AI}-enabled tool that can be used to screen applicants for a job offer by using \acs{LLM}s.
This has the advantage of manageable costs and eliminates the need for historic recruiting data to calibrate the tool as \acs{LLM}s are trained on and have knowledge of a huge corpus of text.
The full version of the problem statement is given in the following.
...
The cost to implement, integrate, and maintain \acs{AI} tools in human resources is significant, may need large amounts of historic recruitment data, and most companies should use services from external providers if they do not have massive amounts of hires to amortize these costs \parencite[8]{ai_recruiting}.
...
The score should represent the candidate's suitability for the job description and enables ordering while the promising flag is needed to quickly filter out unsuitable candidates.
...
The final result is given in percent of correct questions and the authors estimated that human expert level performance is approximately $89.8\%$.
That means if an \acs{LLM} scores higher than the human expert level performance, it can be considered to have the same or superior performance to human domain experts.
...
The \textit{LLAMA 2} model with $70$ billion parameters scored $68.9\%$ on the \textit{MMLU} test \parencite[49]{llama2} which is a considerable worse result that the other two models.
That means all three mentioned models, but especially \textit{Gemini Ultra} and \textit{GPT-4}, have a broad understanding of various domains of knowledge.
This may help the proposed model to accurately match the applicant's \acs{CV} to the job description without the need to supply the model with historic recruitment data.
...
All the presented approaches either used a machine learning model, a text mining or semantical matching approach, or a combination of these approaches to quantize the person-environment fit.
The presented methods all have the common deficiency of either needing to manually create a keyword dictionary or needing to have access to a large dataset of applicant \acs{CV}s and associated job descriptions with past recruitment decisions to train the model.
...
Only in a realistic scenario the model's output can be compared to the human expert recruiters' output.
...
The meetings where the below discussion guides were used were recorded and transcribed in order to get the qualitative data.
...
The two meetings used to get qualitative data were categorized and coded with qualitative content analysis with inductive category formation according to \textit{Mayring} \parencite{mayring}.
...
The transcribed meetings were categorized and coded leading to the resulting data presented below.
...
The results are aggregated leading to the resulting data presented below.

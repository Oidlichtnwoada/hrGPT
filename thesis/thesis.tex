...
\begin{enumerate}
    ...
    phone: +43153534430, mail: katharina.strass@hays.at
    ...
    phone: +4350904199, mail: ams.wien@ams.at
...
Multilingual support for the proposed model can quickly be introduced by translating the extracted non-English text from job descriptions or \acs{CV}s into English using a machine translation service before supplying it to the underlying \acs{LLM}.
This has the advantage that the \acs{LLM}s can still operate in their preferred language domain.
For example, the used machine translation service could be \textcite{deepl_translator}, which also has an API to better integrate with computer programs.
Furthermore, also the English text contents of the \textit{CSV} and \textit{JSON} output files can be translated back to the non-English language of the input \textit{PDF} documents using a machine translation service.

...
Usage of the model should bring efficiency benefits without the need to use it lots of times to amortize the costs, making it perfectly viable for small companies.
...
That means companies of any size can integrate this tool into their human capital sourcing pipelines to increase their efficiency and decrease costs eventually.
...
Complementary fit happens when a person possesses characteristics that are missing in the environment, making it more complete \parencite[180]{po_and_pj_fit_literature_review}.
Complementary person-environment fit encompasses the needs-supplies and demands-abilities perspectives. 
An environment supplies financial, physical, and psychological resources as well as task-related, interpersonal, and growth opportunities that the person demands or needs \parencite[180]{po_and_pj_fit_literature_review}.
...
Furthermore, these three types of fit have a unique impact on various metrics, supporting the previously made claim that they are distinct concepts \parencite[185]{po_and_pj_fit_literature_review}.
...
\begin{itemize}
    ...
    Even though in traditional selection research, the focus was on the person-job fit, the person-organization fit was also already considered in the selection process.
...
\begin{enumerate}
    ...
    A downside of this approach is that the presence of words in resumes not in the predefined keyword dictionary will increase the Euclidian distance even if they may not hurt the person-job fit. 
...
Furthermore, the results heavily depend on the keywords' quality and selection in the keyword dictionary.
...
Of course, the disadvantage of this approach is that the model is only as good as the data it is trained on. For the training, many successful and unsuccessful applications are needed, making this method infeasible for small companies.
...
\end{enumerate}
It was also introduced as a semantic matching system that was used to match the applicant's past experiences to the job requirements, which also incorporated an algorithm that determined whether the past work experience was within the domain of expertise of the job position.
...
\begin{itemize}
    ...
    Of course, all past candidates must again be used to compute the $5$ objective criteria. Also, a human expert recruiter must annotate all the past recruitment decisions with relevant scores that the model should learn. Evidently, this involves a lot of manual labor \parencite[523-524]{applicant_semantic_matching}.
\end{itemize}
...
A focus group of five human expert recruiters will be assembled to analyze the model's output and its impact from a qualitative perspective.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        Furthermore, it is essential to know if these programs support third-party export and import functionalities to use an external program for the screening task.
        ...
        Moreover, the discussion should include these technologies' general reception, advantages, and disadvantages, including potential improvements and drawbacks and their field of use within the human resource pipeline.
        \item \textbf{Costs}
    ...
    For the recruiters to prepare for the meeting, the model's output will be provided to the recruiters beforehand.
    ...
    \begin{enumerate} 
        ...
        If deficiencies in the model are found by inspecting the model's explanations, a discussion on tweaking the model's configuration should be done and whether it is understandable and easy to use.
    \end{enumerate}
\end{enumerate}
...
This split was chosen to have diverse job descriptions with different applicants competing for the job.
...
These samples will be taken with the human expert recruiters at the closing meeting.
...
\end{itemize}
These are why it was decided to construct the sample screening documents for this thesis explicitly.
...
The non-prettified extracted text was already properly usable for the proposed model, but it was decided to prettify the extracted text to group relevant information together.
...
\begin{itemize} \label{job_requirement_types}
    ...
    This requirement type includes knowledge of specific fields necessary to perform the job, which must be listed in the other requirement types.
...
This \textit{JSON} file, compared to the table, additionally contains information on the individual requirements that were matched, including their score and textual explanations.
...
The most extensive job description this thesis uses as input text consists of around $3700$ characters, which would translate to about $925$ tokens.

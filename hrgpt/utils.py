...
def get_applicant_document_paths(filter_job_index: int | None = None,
                                 filter_candidate_index: int | None = None) -> list[str]:
    return [os.path.abspath(x) for x in
            ...
            x.is_file() and
            (filter_job_index is None or f'job_{filter_job_index}' in str(x)) and
            (filter_candidate_index is None or f'candidate_{filter_candidate_index}' in str(x))]

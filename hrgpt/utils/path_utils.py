...
def get_applicant_document_paths(filter_job_indices: tuple[str, ...] = (),
                                 filter_candidate_indices: tuple[str, ...] = ()) -> dict[str, list[str]]:
    ...
    for applicant_document_path in screening_documents_folder_path.rglob('candidate_*.pdf'):
        if not applicant_document_path.is_file():
            continue
        if filter_job_indices != () and all([f'job_{filter_job_index}' not in str(applicant_document_path) for filter_job_index in filter_job_indices]):
            continue
        if filter_candidate_indices != () and all([f'candidate_{filter_candidate_index}' not in str(applicant_document_path) for filter_candidate_index in filter_candidate_indices]):
            continue

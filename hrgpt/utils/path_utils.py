...
def get_applicant_document_paths(
    ...
    for applicant_document_path in screening_documents_folder_path.rglob(
        "candidate_*.pdf"
    ):
        ...
        if filter_candidate_indices != () and all(
            ...
            continue
        job_document_path = os.path.abspath(
            os.path.join(*applicant_document_path.parts[:-2], "job_description.pdf")
        )

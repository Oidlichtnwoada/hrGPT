...
def match_job_requirements_to_candidate_cv(
    ...
    prompts = []
    for requirement_type in AppConfigFactory.get_app_config().generic_config.job_requirements_config.keys():
        for requirement in job_requirements[requirement_type]:
            prompt = get_prompt_to_match_requirement(requirement, requirement_type, cv_text)
    ...
    answer = get_answer_message(get_prompt_to_check_if_candidate_is_promising(requirement_matches))
